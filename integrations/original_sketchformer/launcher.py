...
DEFAULT_CONTINUOUS_RESUME = (
    "weights/pretrained/sketch-transformer-tf2-cvpr_tform_cont/weights/ckpt-12"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build= subparsers.add_parser("build-image", help = "Build the CPU TensorFlow 2.1 Docker image.")
    build.set_defaults(func=build_image)

    build_gpu = subparsers.add_parser("build-gpu-image", help="Build the Tensorflow GPU image intended for RTX 3090 servers.")
    build_gpu.set_defaults(func=build_image, image=DEFAULT_GPU_IMAGE, dockerfile=DEFAULT_GPU_DOCKERFILE, )
    ...
    finetune = subparsers.add_parser("finetune-continuous",
    ...
    finetune.add_argument("--max-seq-len", default=DEFAULT_LEGACY_MAX_SEQ_LEN, type=int,
                          help=(
    ...
    finetune.add_argument("--data-hparams", default=None)
    finetune.add_argument("--base-hparams",
        default = (
            "batch_size=8,num_epochs=1,save_every=1.0,safety_save=1.0,"
            "log_every=10,notify_every=100000,slack_config="
        ),
    )
    finetune.add_argument("--model-hparams",
